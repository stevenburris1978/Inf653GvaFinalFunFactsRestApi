...
const UpdateFunFact = async (req, res) => {
  ...
  // Convert index to zero-based for array manipulation
  const zeroBasedIndex = parseInt(index) - 1;
  
  try {
    const state = await State.findOne({ stateCode: stateCode.toUpperCase() });
    if (!state) {
      // If no existing fun facts, create a new entry
      const newState = new State({
        stateCode: stateCode.toUpperCase(),
        funfacts: [funfact] // Start with the provided fun fact
      });
      await newState.save();
...
// New DELETE handler to remove a specific fun fact based on its index
